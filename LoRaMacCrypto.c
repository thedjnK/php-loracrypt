...
License: Revised BSD License, see LICENSE_SEMTECH file include in the project

Maintainer: Miguel Luis ( Semtech ), Gregory Cristian ( Semtech ) and Daniel Jaeckle ( STACKFORCE )

Modified by thedjnK to reverse encryption and to tidy the formatting up
*/
#include <stdlib.h>
#include <stdint.h>
#include "utilities.h"
...
void LoRaMacPayloadDecrypt(const uint8_t *buffer, uint16_t size, const uint8_t *key, uint32_t address, uint8_t dir, uint32_t sequenceCounter, uint8_t *encBuffer)
{
    uint16_t i;
...
//need to mirror above function
void LoRaMacPayloadEncrypt(const uint8_t *buffer, uint16_t size, const uint8_t *key, uint32_t address, uint8_t dir, uint32_t sequenceCounter, uint8_t *encBuffer)
{
    LoRaMacPayloadDecrypt(buffer, size, key, address, dir, sequenceCounter, encBuffer);
}
...
void LoRaMacJoinDecrypt(const uint8_t *buffer, uint16_t size, const uint8_t *key, uint8_t *encBuffer)
{
    //Used server/gateway side
    memset1(AesContext.ksch, '\0', 240);
    aes_set_key(key, 16, &AesContext);
